"""
E2E tests for post endpoints (consume-style, read-only).

These tests verify that post endpoints work correctly by making
...
def test_get_posts_endpoint():
    """Test GET /api/post endpoint."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/post", headers=headers)
...
def test_get_posts_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/post?name=test", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
def test_get_post_not_found():
    """Test GET /api/post/<id> with non-existent ID."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(
        f"{BASE_URL}/api/post/000000000000000000000000",
        headers=headers,
    )
...
def test_post_endpoints_require_auth():
    ...
    response = requests.get(f"{BASE_URL}/api/post")
