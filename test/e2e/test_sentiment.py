"""
E2E tests for sentiment endpoints.

These tests verify that sentiment endpoints work correctly by making
...
@pytest.mark.e2e
def test_create_sentiment_endpoint():
    """Test POST /api/sentiment endpoint and verify record persists in database."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    data = {
        "name": "e2e-test-sentiment",
        "description": "E2E test sentiment document",
    }

    response = requests.post(f"{BASE_URL}/api/sentiment", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-sentiment"
    assert "created" in response_data
    assert "saved" in response_data


@pytest.mark.e2e
def test_get_sentiments_endpoint():
    """Test GET /api/sentiment endpoint."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/sentiment", headers=headers)
...
def test_get_sentiments_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/sentiment?name=e2e", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
@pytest.mark.e2e
def test_sentiment_endpoints_require_auth():
