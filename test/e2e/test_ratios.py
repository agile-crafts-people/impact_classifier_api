"""
E2E tests for ratios endpoints.

These tests verify that ratios endpoints work correctly by making
...
API runs on port 9102 (same for dev and api).
"""
import pytest
import requests
...
@pytest.mark.e2e
def test_create_ratios_endpoint():
    """Test POST /api/ratios endpoint and verify record persists in database."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    data = {
        "name": "e2e-test-ratios",
        "description": "E2E test ratios document",
    }

    response = requests.post(f"{BASE_URL}/api/ratios", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-ratios"
    assert "created" in response_data
    assert "saved" in response_data


@pytest.mark.e2e
def test_get_ratioss_endpoint():
    """Test GET /api/ratios endpoint."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/ratios", headers=headers)
...
def test_get_ratioss_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/ratios?name=e2e", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
@pytest.mark.e2e
def test_ratios_endpoints_require_auth():
    """Test that ratios endpoints require authentication."""
    response = requests.get(f"{BASE_URL}/api/ratios")
