"""
E2E tests for Ratio endpoints.

These tests verify that Ratio endpoints work correctly by making
...
@pytest.mark.e2e
def test_create_ratio_endpoint():
    """Test POST /api/ratio endpoint and verify record persists in database."""
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    data = {
        "name": "e2e-test-ratio",
        "description": "E2E test ratio document",
    }

    response = requests.post(f"{BASE_URL}/api/ratio", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-ratio"
    assert "created" in response_data
    assert "saved" in response_data


@pytest.mark.e2e
def test_get_ratios_endpoint():
    """Test GET /api/ratio endpoint."""
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/ratio", headers=headers)
...
def test_get_ratios_with_name_filter():
    ...
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/ratio?name=e2e", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
@pytest.mark.e2e
def test_ratio_endpoints_require_auth():
    """Test that ratio endpoints require authentication."""
    response = requests.get(f"{BASE_URL}/api/ratio")
