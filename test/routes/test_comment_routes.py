"""
Unit tests for Comment routes (consume-style, read-only).
"""
import unittest
...
class TestCommentRoutes(unittest.TestCase):
    ...
    def test_get_comments_with_name_filter(
        ...
        """Test GET /api/comment with name query parameter."""
        mock_create_token.return_value = self.mock_token
        mock_create_breadcrumb.return_value = self.mock_breadcrumb

        mock_get_comments.return_value = {
            "items": [{"_id": "123", "name": "test-comment"}],
            "limit": 10,
            "has_more": False,
            "next_cursor": None,
        }

        response = self.client.get("/api/comment?name=test")
    ...
    @patch("src.routes.comment_routes.create_flask_token")
    def test_get_comments_unauthorized(self, mock_create_token):
        """Test GET /api/comment when token is invalid."""
