"""
post service for business logic and RBAC.

Handles RBAC checks and MongoDB operations for post domain.
"""
...
# Allowed sort fields for post domain
...
class postService:
    ...
    - Business logic for post domain (read-only)
    """
    ...
    def _check_permission(token, operation):
        ...
        Example RBAC implementation:
            if operation == 'read':
                ...
                #     raise HTTPForbidden("Insufficient permissions to read post documents")
                pass
        """
        pass
    
    @staticmethod
    def get_posts(token, breadcrumb, name=None, after_id=None, limit=10, sort_by='name', order='asc'):
        """
        Get infinite scroll batch of sorted, filtered post documents.
        ...
        """
        try:
            postService._check_permission(token, 'read')
            mongo = MongoIO.get_instance()
            config = Config.get_instance()
            collection = mongo.get_collection(config.POST_COLLECTION_NAME)
            ...
            logger.info(
                f"Retrieved {len(result['items'])} posts (has_more={result['has_more']}) "
            ...
            raise HTTPInternalServerError("Failed to retrieve posts")
    
    @staticmethod
    def get_post(post_id, token, breadcrumb):
        """
        Retrieve a specific post document by ID.
        ...
        Raises:
            HTTPNotFound: If post is not found
        """
        try:
            postService._check_permission(token, 'read')
            ...
            mongo = MongoIO.get_instance()
            config = Config.get_instance()
            post = mongo.get_document(config.POST_COLLECTION_NAME, post_id)
            if post is None:
                raise HTTPNotFound(f"post { post_id} not found")
            
            logger.info(f"Retrieved post { post_id} for user {token.get('user_id')}")
            return post
        except HTTPNotFound:
            raise
        except Exception as e:
            logger.error(f"Error retrieving post { post_id}: {str(e)}")
            raise HTTPInternalServerError(f"Failed to retrieve post { post_id}")